...
const ALLOWED_TYPES = ["fill_blank", "short_answer", "multiple_choice", "checkbox"];
...
function parseQuestion(row) {
  return {
    ...
    bonnes_reponses: parseJsonArray(row.bonnes_reponses),
...
app.get("/", (req, res) => {
  res.json({
    message: "API Mazavaina Quiz v2.3 types affinés opérationnelle",
    types: {
...
app.get("/questions", (req, res) => {
  db.all(
    `SELECT id, titre, question, type, options, bonnes_reponses, date_creation
     FROM questions
     ORDER BY id ASC`,
    [],
    (err, rows) => {
...
app.post("/admin/questions", requireAdmin, (req, res) => {
  ...
  if (!titre || !question || !normalizedType) {
    return res.status(400).json({
      error: "Le titre, la question et le type sont obligatoires."
    });
  }

  if (!ALLOWED_TYPES.includes(normalizedType)) {
    return res.status(400).json({
      error: "Type invalide. Utilise : fill_blank, short_answer, multiple_choice ou checkbox."
    });
  }

  if (!Array.isArray(bonnes_reponses) || bonnes_reponses.length === 0) {
    return res.status(400).json({
      error: "Au moins une bonne réponse est obligatoire."
    });
  }
  ...
  const cleanAnswers = bonnes_reponses
    .map((item) => String(item).trim())
    .filter(Boolean);

  if ((normalizedType === "multiple_choice" || normalizedType === "checkbox") && cleanOptions.length < 2) {
    return res.status(400).json({
      error: "Les questions à choix doivent avoir au moins deux options."
    });
  }

  if (normalizedType === "multiple_choice" && cleanAnswers.length !== 1) {
    return res.status(400).json({
      error: "Le type choix multiples attend exactement une seule bonne réponse."
    });
  }

  if (normalizedType === "checkbox" && cleanAnswers.length < 1) {
    return res.status(400).json({
      error: "Le type cases à cocher attend au moins une bonne réponse."
    });
  }

  if (normalizedType === "multiple_choice" || normalizedType === "checkbox") {
    const missing = cleanAnswers.filter((answer) => !cleanOptions.includes(answer));
    if (missing.length > 0) {
      return res.status(400).json({
        error: `Bonne réponse absente des options : ${missing.join(", ")}`
      });
    }
  }

  db.run(
    `INSERT INTO questions (titre, question, type, options, bonnes_reponses)
     VALUES (?, ?, ?, ?, ?)`,
    [
      titre.trim(),
      question.trim(),
...
app.delete("/admin/questions", requireAdmin, (req, res) => {
  db.run("DELETE FROM questions", [], function (err) {
    ...
    res.json({
      message: "Toutes les questions ont été supprimées.",
...
app.get("/pseudos/:pseudo", (req, res) => {
  ...
  db.get(
    "SELECT id, pseudo FROM resultats WHERE lower(pseudo) = lower(?) LIMIT 1",
    [pseudo],
    (err, row) => {
      if (err) return res.status(500).json({ error: err.message });
...
app.post("/resultats", (req, res) => {
  ...
  db.get(
    "SELECT id FROM resultats WHERE lower(pseudo) = lower(?) LIMIT 1",
    [cleanPseudo],
    (err, row) => {
      if (err) return res.status(500).json({ error: err.message });
      ...
      if (row) {
        return res.status(409).json({
          error: "Ce pseudo existe déjà. Merci d'en choisir un autre."
        });
      }

      db.run(
        "INSERT INTO resultats (pseudo, score, total) VALUES (?, ?, ?)",
        [cleanPseudo, score, total],
        function (insertErr) {
